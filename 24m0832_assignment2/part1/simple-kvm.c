...
int global_exit_count, io_in_count, io_out_count;
char global_exit_str[128];
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
		{
			...
			char *data_ptr = ((char *)vcpu->kvm_run) + vcpu->kvm_run->io.data_offset;
			// uint32_t size = vcpu->kvm_run->io.size;
			// fprintf(stdout, "%hd\n", port);
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT){
				if(port==0xE9){
					...
					io_out_count++;
					uint32_t guest_ptr;
					memcpy(&guest_ptr, data_ptr, sizeof(guest_ptr));
					char *str = &vm->mem[guest_ptr];
					fprintf(stdout, "%s", str);
					fflush(stdout);
					// continue;
				}
				else if(port==0xEE){
					// TODO: Invalid GVA in protected mode
					global_exit_count++;
					io_out_count++;
					uint32_t gva;
					memcpy(&gva, data_ptr, sizeof(gva));
					uint32_t hva = 0;
					if(gva < vm->mem_size){
						hva = (uint32_t)((uintptr_t)vm->mem + gva);
					}
					else{
						fprintf(stdout, "Invalid GVA\n");
						fflush(stdout);
						hva = 0;
					}
				...
				if(port==0xEB){
					global_exit_count++;
					io_in_count++;
					uint32_t ret = global_exit_count;
                    memcpy(data_ptr, &ret, sizeof(ret));
					// continue;
				}
				else if(port==0xED){
					// TODO: Diff in expected value and my value
					global_exit_count++;
					io_in_count++;
					snprintf(global_exit_str, sizeof(global_exit_str), "IO in: %u\nIO out: %u\n",
						io_in_count, io_out_count);
					uint32_t guest_addr = 0x100000;
					memcpy(&vm->mem[guest_addr], global_exit_str, sizeof(global_exit_str));
					uint32_t ret = guest_addr;
					memcpy(data_ptr, &ret, sizeof(ret));
					// continue;
				}
				else if(port==0xEE){
					...
					uint32_t res = (uint32_t)regs.rax;
					memcpy(data_ptr, &res, sizeof(res));
					// continue;
				}
			}
			continue;
